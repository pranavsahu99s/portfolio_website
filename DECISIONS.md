...
- **Decision:** Call `db.create_all()` and `seed_data()` directly inside `app.py` after all models are defined.
- **Rationale:** When running under Gunicorn on Render as a standard Web Service, Render often ignores `render.yaml` Start Commands in favor of the dashboard UI configuration (`gunicorn app:app`). If `db.create_all()` is placed at the top of the file, models aren't registered yet. By placing the initialization and seeding logic at the bottom of the model definitions in `app.py`, we guarantee that Gunicorn will create the necessary tables and insert default data upon booting the app, regardless of how the start command is configured in the Render dashboard.
